...
class CloudflareService {
  ...
  constructor() {
    ...
    if (!this.apiToken || this.apiToken === "your_api_token_here") {
      throw new Error(
        "Missing or invalid CLOUDFLARE_API_TOKEN in environment variables"
      );
    }
    if (!this.zoneId || this.zoneId === "your_zone_id_here") {
      throw new Error(
        "Missing or invalid CLOUDFLARE_ZONE_ID in environment variables"
      );
    }
  }
  ...
  private async makeRequest<T>(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Cloudflare API error: ${response.status} ${response.statusText}${errorDetails}${troubleshooting}`
      );
    }
  ...
  async createSubdomain(
    ...
    console.log(
      `Creating DNS record: ${subdomain}.${baseDomain} -> ${targetIp}`
    );
    ...
    try {
      ...
      if (!response.success) {
        throw new Error(
          `Failed to create DNS record: ${response.errors
            .map((e) => e.message)
            .join(", ")}`
        );
      }
      ...
      console.log(
        `DNS record created successfully: ${fullDomain} -> ${targetIp}`
      );
  ...
  async createSubdomainWithRetry(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        console.log(
          `Attempting to create subdomain: ${subdomain} (attempt ${
            attempt + 1
          }/${maxRetries})`
        );
        ...
        const isDuplicateError =
          errorMessage?.includes("identical record already exists") ||
          errorMessage?.includes("81058");

        if (isDuplicateError) {
          console.log(
            `Subdomain ${subdomain} already exists, generating a new one...`
          );
          ...
          console.error(
            `Non-retryable error creating subdomain: ${lastError.message}`
          );
          throw lastError;
        }
      }
    ...
    throw new Error(
      `Failed to create subdomain after ${maxRetries} attempts. Last error: ${lastError?.message}`
    );
  }
  ...
  async updateSubdomain(
    ...
    try {
      ...
      if (!response.success) {
        throw new Error(
          `Failed to update DNS record: ${response.errors
            .map((e) => e.message)
            .join(", ")}`
        );
      }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      if (!userResponse.success) {
        console.error(
          "Cloudflare health check failed: Invalid API token or user verification failed"
        );
        return false;
      }
      ...
      if (!zoneResponse.success) {
        console.error(
          "Cloudflare health check failed: Zone access denied or zone not found"
        );
        return false;
      }
  ...
  async generateUniqueSubdomain(): Promise<string> {
    ...
    do {
      ...
      if (attempts > maxAttempts) {
        throw new Error(
          "Unable to generate unique subdomain after multiple attempts"
        );
      }
  ...
  async verifyDNSRecord(
    ...
    try {
      ...
      console.log(
        `Verifying DNS record: ${fullDomain} should point to ${expectedIP}`
      );
      ...
      if (dnsData.Answer && dnsData.Answer.length > 0) {
        ...
        console.log(
          `DNS verification: ${fullDomain} -> ${currentIP} (expected: ${expectedIP})`
        );
        ...
        return {
          ...
          error: success
            ? undefined
            : `IP mismatch: expected ${expectedIP}, got ${currentIP}`,
        };
      } else {
